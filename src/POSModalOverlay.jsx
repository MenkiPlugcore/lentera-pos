...
function FocusButton({ icon, label, active = false, badge = '', onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`relative flex min-h-14 flex-col items-center justify-center gap-0.5 rounded-2xl px-2 py-2 text-center transition duration-200 ${
        active
  ...
  useEffect(() => {
    ...
    Object.assign(aside.style, {
      ...
      transform: modalVisible
        ...
        : 'translateX(-50%) translateY(24px) scale(.97)',
      willChange: 'opacity, transform',
    })
  }, [aside, section, compact, modalMounted, modalVisible])
      ...
      {modalMounted && (
        <>
          <button
            ...
            onClick={closeModal}
            className={`fixed inset-0 z-[89] bg-slate-950/40 backdrop-blur-[3px] transition-opacity duration-300 ${modalVisible ? 'opacity-100' : 'opacity-0'}`}
          />

          <div
            className={`pointer-events-none fixed left-1/2 top-[calc(9vh+12px)] z-[93] flex w-[calc(100%-48px)] max-w-[712px] -translate-x-1/2 items-center justify-between gap-3 transition-all duration-300 ${
